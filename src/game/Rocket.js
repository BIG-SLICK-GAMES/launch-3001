...
export class Rocket {
  ...
  #build() {
    ...
    const nose = new THREE.Mesh(new THREE.ConeGeometry(0.34, 0.64, 18), orangeMat);
    nose.position.y = 1.37;
    this.group.add(nose);

    const booster = new THREE.Mesh(new THREE.CylinderGeometry(0.28, 0.32, 0.42, 16), darkMat);
    ...
    for (let i = 0; i < 3; i += 1) {
      const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.025, 0.75, 8), darkMat);
    ...
    const window = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x5be9ff, emissive: 0x1cc8ff, emissiveIntensity: 1.2, roughness: 0.18 })
    );
